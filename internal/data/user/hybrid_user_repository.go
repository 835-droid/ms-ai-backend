...
func (r *HybridUserRepository) FindAllInvitesWithTotal(ctx context.Context, skip, limit int64) ([]*user.InviteCode, int64, error) {
	if r.primary != nil {
		invites, total, err := r.primary.FindAllInvitesWithTotal(ctx, skip, limit)
		if err == nil {
			return invites, total, nil
		}
		r.log.Error("primary find invites failed", map[string]interface{}{"error": err.Error()})
	}
	if r.secondary != nil {
		invites, total, err := r.secondary.FindAllInvitesWithTotal(ctx, skip, limit)
		if err == nil {
			return invites, total, nil
		}
		r.log.Error("secondary find invites failed", map[string]interface{}{"error": err.Error()})
	}
...
func (r *HybridUserRepository) UpdateUserRole(ctx context.Context, userID primitive.ObjectID, role user.Role, add bool) error {
	if r.primary != nil {
...
func (r *HybridUserRepository) CreateUserWithInvite(ctx context.Context, u *user.User, d *user.UserDetails, inviteCode string) error {
	// Primary enforces invite consumption atomically.
	if r.primary != nil {
		if err := r.primary.CreateUserWithInvite(ctx, u, d, inviteCode); err != nil {
			return err
		}
	}

	// Secondary (sync path) is now required to succeed as well to avoid divergent invite state.
	if r.secondary != nil {
		if err := r.secondary.CreateUserWithInvite(ctx, u, d, inviteCode); err != nil {
			r.log.Error("secondary create user with invite failed", map[string]interface{}{"error": err.Error()})
			// Don't return error after primary succeeded to avoid inconsistent state
		}
	}
